...
<div class="py-12">
            ...
            <div class="flex flex-col">
                <div class="flex flex-row justify-between p-4">
                    <div class="flex flex-row">
                        <input wire:model="search" type="text" placeholder="Search plans..."
                        ...
                        <tr class="hover:bg-gray-100">
                            ...
                            <td class="px-6 py-4 text-sm leading-5 text-gray-900">
                                <div class="flex items-center space-x-2">
                                    <x-secondary-button wire:click="editPlan({{ $plan->id }})">
                                        Edit
                                    </x-secondary-button>
                                    <x-danger-button wire:click="showDeleteModal({{ $plan->id }})">
                                        Delete
                                    </x-danger-button>
                                </div>
                            </td>
    ...
    <div>
        <x-dialog wire:model.live="showModal" maxWidth="5xl" enctype="multipart/form-data">
            ...
            <x-slot name="footer">
                ...
                @if ($planId)
                    <x-primary-button class="ml-4" wire:click="updatePlan" wire:loading.attr="disabled">
                        {{ __('Update') }}
                    </x-primary-button>
                @else
                    <x-primary-button class="ml-4" wire:click="createPlan" wire:loading.attr="disabled">
                        {{ __('Create') }}
                    </x-primary-button>
                @endif
            </x-slot>
    ...
    <div>
        <x-dialog wire:model.live="deleteModal" maxWidth="2xl">
            ...
            <x-slot name="content">
                Are you sure you want to delete this plan <span class="font-semibold text-red-500">
                    {{ $name }}</span>?
            </x-slot>
            <x-slot name="footer">
                ...
                <x-danger-button class="ml-4" wire:click="deletePlan" wire:loading.attr="disabled">
                    {{ __('Delete') }}
                </x-danger-button>
            </x-slot>
        </x-dialog>
